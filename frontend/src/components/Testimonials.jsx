...
export default function Testimonials() {
  ...
  // Create enough duplicates to ensure the marquee container is wider than the screen
  const baseReviews = reviews.length > 0 
    ...
    fetch(`${API_URL}/api/reviews/`)
      ...
      .then((data) => {
        if (data && Array.isArray(data)) {
          ...
          let display = formattedReviews;
          // Duplicate reviews to create enough length for an infinite scroll
          if (display.length > 0) {
            // Duplicate it enough times to ensure it can scroll seamlessly
            while (display.length < 15) {
              display = [...display, ...formattedReviews];
            }
          }
          setReviews(display);
        }
    ...
    <section ref={ref} className="bg-charcoal py-20 md:py-28 px-6 md:px-10 overflow-hidden">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mt-10 overflow-hidden group py-6">
          {reviews.length > 0 ? (
            <div
              className="flex items-stretch w-max marquee-container gap-6 px-6"
              style={{ '--marquee-duration': `${reviews.length * 4.5}s` }}
            >
              <style>
                {`
                  @keyframes marquee {
